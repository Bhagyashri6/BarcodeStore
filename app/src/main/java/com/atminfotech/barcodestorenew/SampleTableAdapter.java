...
public abstract class SampleTableAdapter extends BaseTableAdapter {
	...
	public View getView(final int row, final int column, View converView, ViewGroup parent) {
        ...
        final TextView CellData;

		if (converView == null) {
			converView = inflater.inflate(getLayoutResource(row, column), parent, false);
        }
		setText(converView, getCellString(row, column));
        CellData = (TextView) converView.findViewById(android.R.id.text1);
        CellData.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
				Toast.makeText(context, getCellString(row,column), Toast.LENGTH_SHORT).show();
				/*Intent i = new Intent(getContext(),OrderSummeryForRma.class);
                i.putExtra("date",getCellString(row,-1));
                i.putExtra("billNo",getCellString(row,0));
                i.putExtra("model",getCellString(row,1));
                i.putExtra("serial",getCellString(row,2));
